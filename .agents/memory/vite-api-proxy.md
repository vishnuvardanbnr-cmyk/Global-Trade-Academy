...
`artifacts/edu/vite.config.ts` must include a `server.proxy` block forwarding `/api` to `http://localhost:3000`.
...
**How to apply:** Any time the EDU frontend is touched or a new Vite config is written for this project, confirm the `/api` proxy entry targets port 3000 (API server port). Webview (preview pane) requires the frontend on port 5000; API must be on a different port (3000).
